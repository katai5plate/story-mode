...
export class Script {
  ...
  Head(meta?: Partial<ScriptMetadata>) {
    if (this._calledCount !== 0) throw new Error('$.head は最初に定義してください')
    this._calledCount++
  ...
  Do<A extends keyof CommandsType, B extends keyof CommandsType[A]>(
    ...
    if (!this._isHeadCalled) throw new Error('$.head を最初に必ず呼んでください')
    this._calledCount++
    this._code.push({
      category,
    ...
    new Script(($) =>
      $.Head({ name: 'demo' })
        .Mes({
          ...
          mes: ['ああああああああああああ', 'いいいいいいいいいいいい', 'うううううううううううう']
        })
        .Mes({
          chara: $.REF.Character.Alex,
          mes: [
        ...
        .Do('Audio', 'Play', { media: null as unknown as MusicAsset })
        .Do('Audio', 'Play', { media: null as unknown as SoundAsset })
